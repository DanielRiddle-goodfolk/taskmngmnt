...
async function loadPeople() {
  try {
    ...
    fPerson.innerHTML = '<option value="">Unassigned</option>' +
...
function render() {
  let base = tasks.filter(t => {
    if (currentTab === 'active')   return t.status === 'Not started' || t.status === 'In progress';
    if (currentTab === 'feedback') return t.status === 'Needs Feedback';
    if (currentTab === 'archive')  return t.status === 'Done';
    return true;
  });
...
function openEditMode(id) {
  ...
  const personOpts = PEOPLE.map(p =>
    `<option value="${esc(p.id)}" ${t.person.some(x=>x.id===p.id)?'selected':''}>${esc(p.name)}</option>`
  ).join('');
  detailContent.innerHTML = `
    ...
    <div class="field-row">
      ...
      <div class="field">
        ...
        <select id="e-person"><option value="">Unassigned</option>${personOpts}</select>
      </div>
    </div>
    <div class="field">
  ...
  `;
  // Wire up focus styles for new inputs
...
async function saveEdit(id) {
  ...
  const personEl = document.getElementById('e-person');
  ...
  try {
    const personIds = personEl.value ? [personEl.value] : [];
    ...
    if (t) {
      ...
      const person = PEOPLE.find(p=>p.id===personEl.value);
      t.person   = person ? [{ id: person.id, name: person.name }] : [];
      render();
    }
    closeDetail();
    toast('Task updated ✓');
...
taskForm.addEventListener('submit', async e=>{
  ...
  try {
    const personIds = fPerson.value ? [fPerson.value] : [];
