...
async function loadTasks() {
  try {
    ...
    tasks = data.tasks || data; // support both {tasks:[]} and []
    render();
  } catch(err) {
...
async function saveNotesToNotion(id, notes) {
...
function openDetail(id) {
  ...
  detailContent.innerHTML = `
    <h2 class="sheet-title" style="margin-bottom:.5rem">${esc(t.taskName)}</h2>
...
async function saveNotes(id) {
  ...
  try {
    await saveNotesToNotion(id, newNotes);
...
taskForm.addEventListener('submit', async e=>{
  ...
  try {
    const personOpt = fPerson.options[fPerson.selectedIndex];
